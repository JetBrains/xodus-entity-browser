angular.module('xodus').controller('LinksController',
    ['$scope',
        'entitiesService',
        'EntityTypeService',
        'currentDatabase',
        function ($scope, entitiesService, types, currentDatabase) {
            var linksCtrl = this;

            linksCtrl.entities = [];
            linksCtrl.allEntityTypes = currentDatabase.get().types;
            linksCtrl.newLink = newLink();
            linksCtrl.currentLinks = currentLinks;

            linksCtrl.searchEntities = function (q) {
                types.search(currentDatabase.get(), linksCtrl.newLink.type.id, q, 0, 10).then(function (data) {
            ...
            function newLink() {
                return {
                    name: null,
                    type: currentDatabase.get().types[0],
                    value: null
                }
            }
